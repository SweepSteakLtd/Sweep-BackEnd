...
test('updateCurrentUserHandler - returns 422 if deposit_limit.daily is missing', async () => {
  const res = mockResponse();
  (res as any).locals.user = { id: 'u1', email: 'test@example.com' };

  const req = {
    body: {
      deposit_limit: { weekly: 500, monthly: 1200 },
    },
  } as unknown as Request;
...
test('updateCurrentUserHandler - returns 422 if deposit_limit values are negative', async () => {
  const res = mockResponse();
  (res as any).locals.user = { id: 'u1', email: 'test@example.com' };

  const req = {
    body: {
      deposit_limit: { daily: -10, weekly: 500, monthly: 1200 },
    },
  } as unknown as Request;
...
test('updateCurrentUserHandler - returns 422 if deposit_limit daily exceeds weekly', async () => {
  const res = mockResponse();
  (res as any).locals.user = { id: 'u1', email: 'test@example.com' };

  const req = {
    body: {
      deposit_limit: { daily: 600, weekly: 500, monthly: 1200 },
    },
  } as unknown as Request;
...
test('updateCurrentUserHandler - returns 422 if deposit_limit weekly exceeds monthly', async () => {
  const res = mockResponse();
  (res as any).locals.user = { id: 'u1', email: 'test@example.com' };

  const req = {
    body: {
      deposit_limit: { daily: 100, weekly: 1500, monthly: 1200 },
    },
  } as unknown as Request;
