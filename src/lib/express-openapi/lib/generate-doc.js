  ...
  router && router.stack.forEach((_layer) => {
    iterateStack('', null, _layer, (path, routeLayer, layer) => {
      ...
      console.log('[DEBUG] Processing layer:', {
        ...
        parametersCount: schema.parameters ? schema.parameters.length : 0
      })
      ...
      if (routeLayer && routeLayer.keys && routeLayer.keys.length) {
        console.log('[DEBUG] Processing route with keys:', {
          path,
          keys: routeLayer.keys,
          schema: schema ? { operationId: schema.operationId, parameters: schema.parameters } : null
        })
        const keys = {}
        ...
        const params = routeLayer.keys.map((k, i) => {
          ...
          // Reformat the path
          keys[k.name] = '{' + k.name + '}'
          console.log('[DEBUG] Added key:', k.name, '=', keys[k.name])
        ...
        console.log('[DEBUG] Before pathToRegexp.compile:', {
          originalPath: path,
        ...
        try {
          ...
          console.log('[DEBUG] Path after compile:', path)
        } catch (err) {
          console.error('[DEBUG] Error compiling path:', err.message)
